/**
 * ============================================================
 * NATER LEARNING HUB - ENHANCED SERVER ENGINE v4.0
 * ============================================================
 * Version: 4.0.0 (Complete Static File Serving & Routing Fix)
...
// Serve all static files from the root directory (including logo.jpg)
...
mongoose.connect(process.env.DATABASE_URL, mongoOptions)
    .then(() => {
        ...
        if (isLocalhost) {
            console.log("⚠️  [REMINDER] Check MongoDB Atlas IP Whitelist for localhost access");
        }
    })
    .catch(err => {
        console.error("❌ [CRITICAL] MongoDB Connection Failed:", err.message);
        if (isLocalhost) {
            console.log("🔧 [TROUBLESHOOTING] Check MongoDB Atlas IP Whitelist and network connection");
        }
...
app.post('/api/academy/all-courses', async (req, res) => {
    try {
        ...
        // FIX: Corrected Logic to prevent swapped counts
        const stats = {
            totalCourses: courses.length,
            totalStudents: await User.countDocuments(),
            totalSales: await Access.countDocuments({ 
                payment_reference: { $nin: ['scholarship', 'manual_grant', 'ADMIN', null] } 
            }),
            totalScholarships: await Access.countDocuments({ 
                payment_reference: { $in: ['scholarship', 'manual_grant'] } 
            })
        };
...
// --- FIX: CERTIFICATE VERIFICATION (ID ONLY) ---
app.post('/api/verify-certificate', async (req, res) => {
    try {
        ...
        // Step 1: Find the record by ID only
        const accessRecord = await Access.findOne({ payment_reference: cleanId });

        if (!accessRecord) {
            return res.status(404).json({ 
                success: false, 
                message: "Verification Failed: This Certificate ID is invalid." 
            });
        }

        // Step 2: Automatically find User and Course details from the record
        const course = await Course.findById(accessRecord.course_id);
        const user = await User.findOne({ email: accessRecord.email });
...
// Serve all HTML pages explicitly for better routing
...
app.listen(PORT, () => {
    ...
    console.log(`
    ================================================
    🚀 NATER HUB ENHANCED ENGINE v4.0 ACTIVATED
    📡 PORT: ${PORT}
    🌐 BASE URL: ${baseUrl}
